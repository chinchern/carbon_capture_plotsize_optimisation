...
units = ['MEA_makeup_tank','amine_tank','amine_holding_tank']
Nunits = len(units)

if SwitchSafetyDistances == 1:
    # type of equipment      
   storage_tank =['MEA_makeup_tank','amine_tank','amine_holding_tank']
...
alpha['MEA_makeup_tank'] = 4
alpha['amine_tank'] =10
alpha['amine_holding_tank'] =15

beta['MEA_makeup_tank'] = 4
beta['amine_tank'] = 10
beta['amine_holding_tank'] = 15
...
if SwitchSafetyDistances == 1:
    ...
    for i in storage_tank:
        for j in storage_tank:
            D_min[i][j] = 0.5*(alpha['amine_holding_tank'])
            D_min[j][i]=D_min[i][j]
else:
    for i in units:
        for j in units:
